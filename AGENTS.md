...
- Stable consumer URLs live at the asset root (`/ohrats.css`, `/theme.js`, `/menu.js`, `/logo.svg`, etc.) and serve current bytes directly with revalidation. `/assets/*` is content-fingerprinted and immutable for one year; `/current/*` and `/latest/*` are compatibility-only. Do not use redirects or `no-store` for public static files.
