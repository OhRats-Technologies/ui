...
- `/current/*` is the stable consumer URL and redirects to a content-fingerprinted `/assets/*` file. Stable aliases cache for five minutes and then revalidate; `/assets/*` is immutable for one year. `/latest/*` is compatibility-only. Do not use `no-store` for public static aliases.
